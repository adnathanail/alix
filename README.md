...
- SketchyBar (menu-bar replacement, see `extra/sketchybar/`)
    - FelixKratz's own Lua config (`extra/sketchybar/config/`, vendored from his dotfiles), linked to `~/.config/sketchybar/` — https://felixkratz.github.io/SketchyBar/config. Apple menu, the front app's menus / spaces toggle, media, CPU graph, network up/down + WiFi popup, volume + output-device popup, battery, calendar (click opens Fantastical's Mini Window). The Wi-Fi popup also has an "Open Wi-Fi Settings" row. A menu-bar icon at the far right toggles the native macOS menu bar (auto-hide Always ↔ Never); while it's showing, SketchyBar moves down below it and floats above windows, so the same icon hides it again. The native menu bar also gets a matching icon (`extra/sketchybar/menubar-return.m`, its own launchd agent) that hides it and returns to SketchyBar. Runs on SbarLua; its C helpers are built by Nix. Uses SF Pro + SF Mono (Homebrew `font-sf-pro` / `font-sf-mono`), SwitchAudioSource and nowplaying-cli. Clicking a space does nothing (it calls `yabai`, which isn't installed). The app-menu helper needs SketchyBar's Accessibility grant (below)
