    ...
    darwinConfigurations.${hostname} = nix-darwin.lib.darwinSystem {
      modules = [
        ...
        ({ pkgs, ... }: {
          ...
          # After agenix decrypts to /run/agenix/*, materialise the secrets
          # we want in the shell as a plain env file. Sourced from zsh's
          # initContent so tokens end up in $NPM_*_TOKEN without doing a
          # per-shell file read of each individual secret. `postActivation`
          # runs at the tail of `darwin-rebuild switch`, i.e. after agenix.
          # Add another secret: append another `write NAME PATH` line.
          system.activationScripts.postActivation.text = ''
            ...
            tmp="$(mktemp)"
            {
              ...
              echo "# Do not edit; change secrets/ + flake.nix instead."
              write() {
